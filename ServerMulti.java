...
public class ServerMulti {
    ...
    public static void main(String[] args) throws IOException {
        Map addressBookList = new HashMap<String, ClientHandlerMulti.AddressBook>();
        ...
        while (true) {
            ...
            try {
                ...
                Thread t = new ClientHandlerMulti(socket, dis, dos, addressBookList);
                clientList.add(t);
                t.start();
...
class ClientHandlerMulti extends Thread {
    ...
    Map users = new HashMap<String, ClientHandlerMulti.UserLogin>();
    File addressBook = null;

    // Constructor
    public ClientHandlerMulti(Socket s, DataInputStream dis, DataOutputStream dos, Map addressBookList) {
    ...
    private void loadLogin() {
        ClientHandlerMulti.UserLogin userLogin = new UserLogin();
                ...
                switch (command) {
                    case "ADD":
                        if(isUserLogin) {
                            ...
                            dos.writeUTF("s:The new Record ID is " + ServerMulti.maxRecordId);
                        } else {
                    ...
                    case "LIST":
                        ...
                        for (Object entry : addressBookList.values()) {
                            dos.writeUTF(((ClientHandlerMulti.AddressBook) entry).getRecordId() + " " + ((ClientHandlerMulti.AddressBook) entry).getFirstName() + " " + ((ClientHandlerMulti.AddressBook) entry).getLastName() + " " + ((ClientHandlerMulti.AddressBook) entry).getPhoneNumber());
                        }
                        break;
                    case "SHUDDOWN":
                        if(isUserLogin) {
                            dos.writeUTF("s:200 OK");
                            for(Thread t: ServerMulti.clientList) {
                                ClientHandlerMulti cl = (ClientHandlerMulti) t;
    ...
    public static Map<String, ClientHandlerMulti.AddressBook> add(Map addressBookList, String record) {
        ClientHandlerMulti.AddressBook addressBook1 = new ClientHandlerMulti.AddressBook();
        ServerMulti.maxRecordId = ServerMulti.maxRecordId + 1;
        addressBook1.setRecordId(ServerMulti.maxRecordId);
        ...
        try {
            ...
            while ((line = is.readLine()) != null) {
                if(!line.startsWith("RecordID")) {
                    ClientHandlerMulti.AddressBook record = new ClientHandlerMulti.AddressBook();
            ...
            ServerMulti.maxRecordId = maxRecordIdInput;
        } catch (IOException e) {
    ...
    public static void writeAddressBook(Map addressBookList, File addressBook) {
        ...
        try {
            ...
            for (Object entry : addressBookList.values()) {
                line = ((ClientHandlerMulti.AddressBook) entry).getRecordId() + " " + ((ClientHandlerMulti.AddressBook) entry).getFirstName() + " " + ((ClientHandlerMulti.AddressBook) entry).getLastName() + " " + ((ClientHandlerMulti.AddressBook) entry).getPhoneNumber();
                os.write(line + "\n");
            }
            os.close();
        } catch (IOException e) {
